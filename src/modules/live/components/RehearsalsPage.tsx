...
export function RehearsalsPage() {
    const { rehearsals, productions, loading, error, sliceError } = useLiveData();
    const params = useSearchParams();
    const [period, setPeriod] = useState("upcoming");
    const [search, setSearch] = useState("");
    const [tour, setTour] = useState(params.get("tourId") ?? "");
    const [showId, setShowId] = useState(params.get("productionId") ?? "");
    if (loading)
        return <PageLoader />;
    const loadError = sliceError("rehearsals");
    if (loadError)
        return <PageError title="Impossible de charger tes répétitions" description={loadError} onRetry={() => mutate("user_live")}/>;
    const upcoming = rehearsals.filter(r => dateISO(r.date) >= todayISO());
    const shown = rehearsals.filter(r => (period === "past" ? dateISO(r.date) < todayISO() : dateISO(r.date) >= todayISO()) && (!showId || r.details?.productionId === showId) && (!tour || r.details?.tourId === tour) && `${r.label} ${r.location} ${r.city}`.toLocaleLowerCase().includes(search.toLocaleLowerCase())).sort((a, b) => period === "past" ? dateISO(b.date).localeCompare(dateISO(a.date)) : dateISO(a.date).localeCompare(dateISO(b.date)));
    return <div>
    ...
    <Link href="/live">Mes spectacles <ArrowUpRight size={13} className="ml-2"/></Link>
    </Button>
    </div>
    ...
    <div className="w-56"><Choice label="Spectacle" optional noneLabel="Tous" value={showId} onChange={v => { setShowId(v); setTour(""); }} options={showsOf(productions).map(p => ({ value: p.id, label: p.title }))}/></div>
    <div className="w-56"><Choice label="Tournée" optional noneLabel="Toutes" value={tour} onChange={setTour} options={tourOptions(productions, showId || undefined)}/></div>
